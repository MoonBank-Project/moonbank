...
/* void Updater::checkForUpdate()
{
  ...
  const QUrl url = QUrl::fromUserInput("https://raw.githubusercontent.com/MoonBank-core/Assets/master/version.txt"); // todo
  ...
  else
  {
    ...
    if (ourVersion < remoteVersion)
    {
      if (QMessageBox::warning(
              &MainWindow::instance(), QObject::tr("MoonBank Wallet Update"),
              QObject::tr("There is an update to the wallet available.\nWould you like to go to "
                          "the download page?"),
              QMessageBox::Ok, QMessageBox::Cancel) == QMessageBox::Ok)
      {
        QString link = "https://github.com/MoonBank-Project/moonbank-desktop/releases";
