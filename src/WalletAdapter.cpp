...
bool WalletAdapter::save(bool _details, bool _moonbank) {
  return save(Settings::instance().getWalletFile() + ".temp", _details, _moonbank);
}

bool WalletAdapter::save(const QString& _file, bool _details, bool _moonbank) {
  Q_CHECK_PTR(m_wallet);
  if (openFile(_file, false)) {
    try {
      m_wallet->save(m_file, _details, _moonbank);
    } catch (std::system_error&) {
      closeFile();
      return false;
    }
